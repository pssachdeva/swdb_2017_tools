...
def shuffle_intervals(spkts,T,dt,binsize): 
    ...
    for cellID in range(num_cells): 
        if np.isfinite(spkts[cellID,:]).any()==0:
            #nospks_ind = np.append(nospks_ind,cellID)    # indices of cellIDs with no spikes
            continue
    ...
    return time_resamp, ctrlhist_binned #, nospks_ind
...
def find_high_activity(spktms, T, dt, binsize, nsurr=1000, pval = 0.05):
    ...
    coactive_surr_binned_tot = np.zeros(coactive_binned.shape)
    ...
    for _ in range(nsurr):
        ...
        coactive_surr_binned_tot += coactive_surr_binned
    ...
    num_ensembles = ensembles_w_cells.shape[1]   
    percent_ensembles = float(num_ensembles)/ensembles.shape[0]
    
    return time_w_ensembles, ensembles_w_cells, percent_ensembles, coactive_binned, coactive_surr_binned_tot
